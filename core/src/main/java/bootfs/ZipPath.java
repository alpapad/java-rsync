...
final class ZipPath implements Path {
    ...
    private final SpringBootFileSystem zfs;
    
    ZipPath(SpringBootFileSystem zfs, byte[] path) {
        this(zfs, path, false);
    }
    
    ZipPath(SpringBootFileSystem zfs, byte[] path, boolean normalized) {
        this.zfs = zfs;
        if (normalized) {
            this.path = path;
        } else {
    ...
    ZipPath(SpringBootFileSystem zfs, String path) {
        this.zfs = zfs;
        this.path = normalize(path);
    }
    ...
    void copy(ZipPath target, CopyOption... options) throws IOException {
        if (Files.isSameFile(zfs.getZipFile(), target.zfs.getZipFile())) {
    ...
    public boolean equals(Object obj) {
        return obj != null && obj instanceof ZipPath && zfs == ((ZipPath) obj).zfs && compareTo((Path) obj) == 0;
    }
    ...
    FileStore getFileStore() throws IOException {
        // each SpringBootFileSystem only has one root (as requested for now)
        if (exists()) {
    ...
    @Override
    public SpringBootFileSystem getFileSystem() {
        return zfs;
    }
    ...
    private byte[] getResolved() {
        for (int i = 0; i < path.length; i++) {
            if (path[i] == (byte) '.' && (i + 1 == path.length || path[i + 1] == '/')) {
                return resolve0();
            }
        }
        return path;
    ...
    OutputStream newOutputStream(OpenOption... options) throws IOException {
        if (options.length == 0) {
            return zfs.newOutputStream(getResolvedPath(), CREATE, TRUNCATE_EXISTING, WRITE);
        }
    ...
    public Path relativize(Path other) {
        ...
        if (zfs != o.zfs || isAbsolute() != o.isAbsolute()) {
            throw new IllegalArgumentException();
        }
        if (path.length == 1 && path[0] == '/') {
            return new ZipPath(zfs, Arrays.copyOfRange(o.path, 1, o.path.length), true);
        }
        int mc = getNameCount();
