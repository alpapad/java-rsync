...
/**
 * Adds aliasing to SpringBootFileSystem to support multi-release jar files. An alias
 * map is created by {@link SpringBootJarFileSystem#createVersionedLinks(int)}. The map is
 * then consulted when an entry is looked up in
 * {@link SpringBootJarFileSystem#getEntry(byte[])} to determine if the entry has a
 * corresponding versioned entry. If so, the versioned entry is returned.
 *
 * @author Steve Drach
 */

class SpringBootJarFileSystem extends SpringBootFileSystem {
    private Function<byte[], byte[]> lookup;

    SpringBootJarFileSystem(SpringBootFileSystemProvider provider, Path zfpath, Map<String, ?> env) throws IOException {
