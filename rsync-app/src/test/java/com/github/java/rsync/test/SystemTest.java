...
class TestSimpleModule implements Module {
    ...
    TestSimpleModule(String name, String root, String comment, boolean isReadable, boolean isWritable) {
...
class SimpleRestrictedModule extends RestrictedModule {
    ...
    public String authenticate(RsyncAuthContext authContext, String userName) {
        return authContext.response(authToken.toCharArray());
    }
...
public class SystemTest {
    ...
    public void testClientCopyPreserveGid() throws IOException {
        ...
        Path src = tempDir.newFolder().toPath();
        ...
        ReturnStatus status = fileCopy(src, dst, "--recursive", "--group", "--numeric-ids");
    ...
    public void testClientDotDotSrcArg() throws IOException {
        Path src = tempDir.newFile().toPath();
        Path srcDotDot = tempDir.newFolder().toPath().resolve(Paths.get(Text.DOT_DOT)).resolve(src.getFileName());
        Path dst = tempDir.newFile().toPath();
    ...
    public void testFileUtilIdenticalDirsWithSymlinks() throws IOException {
        Path left = tempDir.newFolder("left_dir").toPath();
        Path right = tempDir.newFolder("right_dir").toPath();
    ...
    public void testFileUtilIdenticalEmptyFiles() throws IOException {
        Path left = tempDir.newFile("left_file").toPath();
        Path right = tempDir.newFile("right_file").toPath();
    ...
    public void testInvalidPassword() throws InterruptedException {
        ...
        Callable<Integer> serverTask = new Callable<Integer>() {
            ...
            public Integer call() throws Exception {
                ...
                Module m = new TestSimpleModule(restrictedModuleName, modulePath.toString(), "a test module", true, false);
        ...
        YajsyncClient client = newClient().setStandardOut(nullOut);
    ...
    public void testLocalListDotDir() throws IOException {
        Path dir = tempDir.newFolder("dir").toPath();
    ...
    public void testServerConnection() throws InterruptedException {
        ...
        Callable<Integer> serverTask = new Callable<Integer>() {
            ...
            public Integer call() throws Exception {
                Path modulePath = tempDir.newFolder().toPath();
                Module m = new TestSimpleModule("test", modulePath.toString(), "a test module", true, false);
    ...
    public void testServerHelp() throws InterruptedException, IOException {
        int rc = newServer(new TestModules()).setStandardOut(nullOut).start(new String[] { "--help" });
        assertTrue(rc == 0);
    }
    ...
    public void testSiblingSubdirsSubstring() throws IOException {
        Path src = tempDir.newFolder().toPath();
        Path dst = tempDir.newFolder().toPath();
