...
public final class Util {
     ...
     */
    public static ByteBuffer enlargeByteBuffer(ByteBuffer src, MemoryPolicy policy, int maxSize) {
     ...
     * @throws OverflowException if enlarged buffer would be greater than maxSize
     */
    public static CharBuffer enlargeCharBuffer(CharBuffer src, MemoryPolicy policy, int maxSize) {
        int nextSize = src.capacity() * 2;
        if (nextSize <= 0 || nextSize > maxSize) {
            throw new OverflowException(String.format("allocation limit exceeded max is %d", maxSize));
        }
