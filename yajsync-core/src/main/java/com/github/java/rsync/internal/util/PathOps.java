...
public final class PathOps {
    ...
    public static Path get(FileSystem fs, String name) {
        ...
        } else if (name.endsWith(Text.SLASH) || name.endsWith(Text.SLASH + Text.DOT)) {
    ...
    private static boolean isWindowsPathPreserved(Path path) {
        ...
        for (Path p : path) {
            ...
            if (name.endsWith(Text.DOT) && !name.equals(Text.DOT) && !name.equals(Text.DOT_DOT)) {
                return false;
            }
        }
        return true;
    ...
    private static Path normalizePathWin(Path path) {
        ...
        for (Path p : path) {
            if (p.equals(dotDotDir)) {
                ...
                String normalized = Text.deleteTrailingDots(p.toString()).toLowerCase();
    ...
    public static Path subtractPathOrNull(Path parent, Path sub) {
        if (!parent.endsWith(sub)) {
            throw new IllegalArgumentException(String.format("%s is not a parent path of %s", parent, sub));
        }
        if (parent.getNameCount() == sub.getNameCount()) {
            return parent.getRoot(); // NOTE: return null if parent has no root
        }
        Path res = parent.subpath(0, parent.getNameCount() - sub.getNameCount());
